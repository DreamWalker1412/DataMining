...
def aprioriHash(dataSet, minSupport):
    ...
    while len(frequentList[k - 1]) > 0:
        candidateK, count = candidateGenerate(frequentList[k - 1], k + 1, hashTable, minSup, order)  # 根据K频繁项集生成K+1候选项集
        countAll += count
...
if __name__ == '__main__':
    ...
    # 调用算法
    frequentList, supportDictionary = aprioriHash(dataSet1, minSupport)
